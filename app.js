...
// Funzione per comprimere immagini (canvas)
...
// Funzione per "comprimere" un PDF usando pdf-lib
async function compressPDF(file) {
  const arrayBuffer = await file.arrayBuffer();
  const pdfDoc = await PDFDocument.load(arrayBuffer);
  // Salviamo il PDF; pdf-lib effettua alcune ottimizzazioni per impostazione predefinita.
...
async function loadFattureAdmin() {
  ...
  try {
    ...
    snap.forEach((docSnap) => {
      ...
      // Bottone per visualizzare e scaricare la fattura
      ...
      // Bottone per cancellare la fattura
...
// Utility: Comprimi l'immagine usando un canvas
function compressImage(file, maxWidth, maxHeight, quality) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    const reader = new FileReader();
...
// Utility: Comprimi un PDF usando pdf-lib
async function compressPDF(file) {
  const arrayBuffer = await file.arrayBuffer();
  const pdfDoc = await PDFDocument.load(arrayBuffer);
  // Rielabora il PDF (pdf-lib effettua alcune ottimizzazioni)
  const compressedBytes = await pdfDoc.save();
  let binary = "";
  const bytes = new Uint8Array(compressedBytes);
  bytes.forEach(b => binary += String.fromCharCode(b));
...
// Utility: Scarica il file (immagine o PDF) come file
function downloadFattura(base64Data, filename) {
  const a = document.createElement("a");
  a.href = base64Data;
  a.download = filename;
