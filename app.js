...
// Admin: Visualizza Prenotazioni dei clienti
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    currentUser = user;
    // Legge il documento in Firestore per il ruolo
    const userDocRef = doc(db, "users", user.uid);
    const userDocSnap = await getDoc(userDocRef);
    if (userDocSnap.exists()) {
      currentUserRole = userDocSnap.data().role;
    } else {
      // Se non esiste, crea il doc con ruolo "cliente" di default
...
// ADMIN: Aggiungi Fattura per cliente (file in Base64)
...
async function loadFattureAdmin() {
  ...
  try {
    ...
    snap.forEach((docSnap) => {
      ...
      // Bottone per visualizzare la fattura
